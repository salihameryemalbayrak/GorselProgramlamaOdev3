...
public partial class Yapilacaklar : ContentPage, INotifyPropertyChanged
{
    ...
    public Yapilacaklar()
    {
        ...
        // Firebase init (giriş yaptıysan Preferences'tan çeker)
    ...
    {
        // Login/Register sonrası kaydedilen UserId varsa Firebase'i başlat
    ...
    // ✅ DÜZELTİLEN KISIM: FireBaseServices.GetAllGorevlerAsync() kullanılıyor
    private async Task LoadGorevlerAsync()
    {
        try
        {
            // UserId boşsa kullanıcı henüz giriş yapmamış olabilir
            var userId = Preferences.Get("UserId", "");
            if (string.IsNullOrWhiteSpace(userId))
            {
                Gorevler.Clear();
            ...
            // Firebase init garantile
            FireBaseServices.Initialize(userId);

            var gorevler = await FireBaseServices.GetAllGorevlerAsync();
    ...
    private async void OnGorevSilClicked(object sender, EventArgs e)
    {
        if (sender is Button button && button.CommandParameter is Gorev gorev)
        {
            bool result = await DisplayAlert(
                "Silinsin mi?",
                $"'{gorev.Baslik}' görevini silmek istediğinizden emin misiniz?",
                "Evet",
                "Hayır"
            );
    ...
    protected override void OnAppearing()
    {
        ...
        LoadThemeSettings(); // Tema ayarlarını yeniden yükle
        EnsureFirebaseInitialized();
